...
import { Plus, RefreshCw, Edit, Trash2, Eye, ChevronLeft, ChevronRight, MoreVertical, Package, Sofa } from "lucide-react";
...
import AddFurnitureProductModal from "./AddFurnitureProductModal";

// Furniture Product Card Component
interface FurnitureProductCardProps {
  product: IFurnitureProduct;
  onEdit: (product: IFurnitureProduct) => void;
  onDelete: (productId: number) => void;
  onView: (product: IFurnitureProduct) => void;
}

export const FurnitureProductCard: React.FC<FurnitureProductCardProps> = ({ product, onEdit, onDelete, onView }) => {
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden border border-slate-200 hover:shadow-lg transition-all duration-300">
      ...
      <div className="relative h-56 overflow-hidden">
        {allImages.length > 0 ? (
                ...
                {allImages.map((_, index) => (
                  <button
                    ...
                    className={`h-2 w-2 rounded-full ${
                      index === currentImageIndex ? 'bg-slate-900' : 'bg-white/70'
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="w-full h-full bg-slate-100 flex items-center justify-center">
            <Sofa className="h-12 w-12 text-slate-400" />
          </div>
        )}
        ...
        {/* Brand Badge */}
        {product.brand && (
          <div className="absolute top-3 right-12 px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
            {product.brand}
          </div>
        )}
        ...
        <div className="absolute top-3 right-3">
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-1 w-32 bg-white rounded-md shadow-lg py-1 z-10 border border-slate-200">
              <button
                onClick={(e) => { e.stopPropagation(); onView(product); setShowDropdown(false); }}
              ...
              <button
                onClick={(e) => { e.stopPropagation(); onEdit(product); setShowDropdown(false); }}
                className="flex items-center w-full px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
              >
              ...
              <button
                onClick={(e) => { e.stopPropagation(); onDelete(product.id); setShowDropdown(false); }}
      ...
      <div className="p-4">
        ...
        <div className="mt-2 space-y-1">
          ...
          {product.dimensions && (
            <div className="text-xs text-slate-600">
              Dimensions: <span className="font-medium">{product.dimensions.length}x{product.dimensions.width}x{product.dimensions.height} {product.dimensions.unit}</span>
            </div>
          )}
        </div>
...
// Main Furniture Products Component
export default function FurnitureProducts() {
  const [showAddFurnitureProductModal, setShowAddFurnitureProductModal] = useState(false);
  const [products, setProducts] = useState<IFurnitureProduct[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [editingProduct, setEditingProduct] = useState<IFurnitureProduct | null>(null);
  const [viewingProduct, setViewingProduct] = useState<IFurnitureProduct | null>(null);
  ...
  const fetchFurnitureProducts = async () => {
    try {
      ...
      setError("Failed to fetch furniture products. Please try again.");
  ...
  const handleEditProduct = (product: IFurnitureProduct) => {
    setEditingProduct(product);
    setViewingProduct(null);
    setShowAddFurnitureProductModal(true);
  };

  const handleViewProduct = (product: IFurnitureProduct) => {
  ...
  const handleDeleteProduct = async (productId: number) => {
    if (deleteConfirm === productId) {
      try {
        await FURNITURE_API.DELETE_FURNITURE_PRODUCT(productId);
        setProducts(products.filter(p => p.id !== productId));
        setDeleteConfirm(null);
      } catch (err) {
        setError("Failed to delete furniture product. Please try again.");
  ...
  const handleProductCreated = () => {
    ...
    setEditingProduct(null);
    setViewingProduct(null);
    setShowAddFurnitureProductModal(false);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Manage your furniture inventory ({products.length} products)
          </p>
        </div>
        <div className="flex items-center gap-3">
          <ActionButton 
            onClick={fetchFurnitureProducts} 
            variant="secondary" 
            className="text-sm"
            disabled={loading}
          >
          ...
          <ActionButton
            onClick={() => setShowAddFurnitureProductModal(true)}
            variant="primary"
            className="text-sm"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-xl border border-dashed border-slate-300">
          <Sofa className="h-12 w-12 text-slate-400 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-slate-900">No furniture products yet</h3>
          <p className="text-slate-600 mt-1 mb-4">Get started by adding your first furniture product</p>
          <ActionButton
            onClick={() => setShowAddFurnitureProductModal(true)}
            variant="primary"
            className="text-sm"
            ...
            <div className="fixed inset-0 bg-slate-900/70 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-xl p-6 max-w-md w-full">
                ...
                <p className="text-slate-600 mb-6">
                  Are you sure you want to delete this furniture product? This action cannot be undone.
                </p>
      ...
      <AddFurnitureProductModal
        isOpen={showAddFurnitureProductModal}

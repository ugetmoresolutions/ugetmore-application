...
import brandingBanner from "@/public/yougetmore assets/pictures/technology.png";
import {
  ShoppingCart,
  AlertCircle,
  Filter,
...
// Main Electronics Component - UPDATED TO USE SERVER-SIDE FILTERING
export default function Electronics() {
  ...
  const getCategoryDisplayName = (categoryName: string): string => {
    const displayNameMap: { [key: string]: string } = {
      ...
      // Add other display name mappings here if needed in the future
    };

    return displayNameMap[categoryName] || categoryName;
  };

  // NEW: Fetch filtered products from backend with all filters
  const fetchFilteredProducts = async (search: string = searchTerm) => {
    try {
      ...
      console.log(
        "🔧 Fetching filtered Tarsus products with filters:",
        filters
      );

      const response =
        await AGGREGATED_PRODUCTS_API.GET_TARSUS_PRODUCTS_WITH_FILTERS(filters);
      ...
      setErrorState(`Failed to fetch electronics products: ${err.message}`);
      setProducts([]);
      setTotalProducts(0);
      setTotalPages(0);
    } finally {
  ...
  // localStorage keys
const CATEGORIES_STORAGE_KEY = 'tarsus_categories';
const CATEGORIES_TIMESTAMP_KEY = 'tarsus_categories_timestamp';
const CACHE_DURATION = 1000 * 60 * 60 * 24; // 24 hours in milliseconds

// Fetch Tarsus categories from the new endpoint with localStorage caching
const fetchTarsusCategories = async () => {
  try {
    ...
    console.log("🔧 Fetching fresh Tarsus categories from API...");
    const response = await AGGREGATED_PRODUCTS_API.GET_TARSUS_CATEGORIES();

    if (response?.data) {
      console.log("✅ Categories fetched:", response.data.categoriesWithSubs);
      ...
      // Fallback: try to use cached data even if expired, or extract from products
      if (cachedCategories) {
...
const refreshCategories = async () => {
  ...
  await fetchTarsusCategories();
};

  // Fallback method to extract categories from products
  const extractCategoriesFromProducts = () => {
    console.log("🔧 Extracting categories from products as fallback...");
    ...
    products.forEach((product) => {
      product.categories?.forEach((cat: any) => {
        ...
        if (categoryName && !seen.has(categoryName)) {
          ...
          cats.push({
            name: categoryName,
            subCategories: null, // Tarsus typically doesn't have subcategories
          });
        }
      });
    });
    ...
    console.log("🔧 Extracted categories from products:", allCategories);

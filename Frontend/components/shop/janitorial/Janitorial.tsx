...
// Main Janitorial Component - UPDATED TO USE PARROT JANITORIAL ENDPOINT
export default function Janitorial() {
  ...
  // NEW: Fetch filtered janitorial products from backend with all filters
  const fetchFilteredProducts = async (search: string = searchTerm) => {
    try {
      ...
      console.log(
        "🔧 Fetching filtered Parrot janitorial products with filters:",
        filters
      );

      const response =
        await AGGREGATED_PRODUCTS_API.GET_PARROT_JANITORIAL_PRODUCTS(filters);
      ...
      if (response.data) {
        ...
        console.log(
          "✅ Filtered janitorial products fetched:",
      ...
      setErrorState(`Failed to fetch janitorial products: ${err.message}`);
      setProducts([]);
      setTotalProducts(0);
      setTotalPages(0);
    } finally {
  ...
  // localStorage keys
  const CATEGORIES_STORAGE_KEY = 'parrot_janitorial_categories';
  const CATEGORIES_TIMESTAMP_KEY = 'parrot_janitorial_categories_timestamp';
  const CACHE_DURATION = 1000 * 60 * 60 * 24; // 24 hours in milliseconds

  // Fetch Parrot janitorial categories from the new endpoint with localStorage caching
  const fetchJanitorialCategories = async () => {
    try {
      ...
      if (cachedCategories && cachedTimestamp) {
        ...
        if (cacheAge < CACHE_DURATION) {
          console.log("📦 Using cached janitorial categories from localStorage");
      ...
      console.log("🔧 Fetching fresh Parrot janitorial categories from API...");
      const response = await AGGREGATED_PRODUCTS_API.GET_PARROT_JANITORIAL_CATEGORIES();

      if (response?.data) {
        console.log("✅ Janitorial categories fetched:", response.data.categoriesWithSubs);
        ...
        console.log("✅ Janitorial categories set and cached successfully:", transformedCategories.length);
      } else {
      ...
      console.error("❌ Error fetching janitorial categories:", error);
  ...
  const clearCategoriesCache = () => {
    ...
    console.log("🗑️ Janitorial categories cache cleared");
  };

  // Optional: Add a function to force refresh categories
  const refreshCategories = async () => {
    clearCategoriesCache();
    await fetchJanitorialCategories();
  };
  ...
  // Debug useEffect
  useEffect(() => {
    console.log("🔧 Janitorial products state updated:", {
    ...
    if (products.length > 0) {
      console.log("🔧 Sample janitorial product structure:", {
  ...
  return (
    <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <form onSubmit={handleSearchSubmit} className="flex-1 relative">
              <input
                ref={searchInputRef}
                type="text"
                placeholder="Search janitorial supplies..."
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
                  <div className="text-xs sm:text-sm text-gray-500 order-2 sm:order-1">
                    Showing {products.length} of {totalProducts} janitorial products
                    {searchTerm ? (
      ...
      <div className="relative w-full h-48 sm:h-64 lg:h-80 xl:h-[500px] overflow-hidden">
        <Image
          src={brandingBanner}
          alt="Janitorial Supplies Collection"
          fill
